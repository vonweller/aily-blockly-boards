...
- 上游：[arduino-stc51](https://github.com/coloz/arduino-stc51)，FQBN：`arduino-stc51:mcs51:stc8g1k08`。
- 型号与资源：`tools/variants/devices.json`、`boards.txt`。
- 引脚：`variants/STC8G1K08/pins_arduino.h` 和 `variants/_common/pins_arduino_common.h`。
- [原厂型号资料](https://www.stcmicro.com/cn/stc/stc8g1k08.html)。
- 18 个逻辑 GPIO，15 个模拟输入。引脚值使用上游 `P3_2` 等宏；底层编码为 `(port << 4) | bit`，不是连续数字。
