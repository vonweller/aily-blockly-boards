...
```
avrdude -C${'avrdude.conf'} -v -V -patmega328p -carduino -P${serial} -b${baud} -D -Uflash:w:${'*.bin'}:i;

esptool --chip esp32s3 --port ${serial} --baud ${baud} --before default_reset --after hard_reset write_flash -z --flash_mode keep --flash_freq keep --flash_size keep 0x0 ${bootloader} 0x8000 ${partitions} 0xe000 ${boot_app0} 0x10000 ${'*.merged.bin'}"

bossac -d --port=${serial} -a -U -e -w ${'*.hex'} -R
[--use_1200bps_touch, --wait_for_upload] bossac -d --port=${serial} -a -U -e -w ${'*.hex'} -R

dfu-util --device 0x2341:0x0069,:0x0369 -D ${'*.hex'} -a0 -Q
```
